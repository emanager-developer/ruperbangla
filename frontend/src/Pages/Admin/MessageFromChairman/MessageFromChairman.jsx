...
} from "../../../Redux/director/mfChairman";
...
export default function MessageFromChairman() {
  ...
  const [addMFChairman, { isLoading: isAddLoading }] =
    useAddMFChairmanMutation();
  const [updateMFChairman, { isLoading: isULoading }] =
    useUpdateMFChairmanMutation();

  useEffect(() => {
    if (mfChairMan) {
      setTitle(mfChairMan.name);
      setDescription(mfChairMan.message);
      setImage([
        { data_url: `${import.meta.env.VITE_BACKEND_URL}${mfChairMan.image}` },
      ]);
    }
  }, [mfChairMan]);
  ...
  const handleAddEdit = async (e) => {
    ...
    if (mfChairMan?._id) {
      const res = await updateMFChairman({
        id: mfChairMan?._id,
        formData,
      });

      if (res?.data?.success) {
        Swal.fire("", "Message From Chairman updated successfully", "success");
        setTitle("");
        setDescription("");
        setImage([]);
      } else {
      ...
      const res = await addMFChairman(formData);
  ...
  return (
    <section className="rounded bg-base-100 shadow">
      <div className="border-b p-4">
        <h3 className="font-medium text-neutral">Message From Chairman</h3>
      </div>
